...
async def get_all_levels_view(db: AsyncSession = Depends(get_db)) -> list[EducationalLevelSchema]:
    return await get_all_educational_levels(db)


@router.get("/educational-levels/{id}/", response_model=EducationalLevelSchema)
async def get_level_view(id_: int, db: AsyncSession = Depends(get_db)) -> EducationalLevelSchema:
...
async def get_groups_by_educational_level_view(id: int, db: AsyncSession = Depends(get_db)) -> Iterable[Group]:
    return await get_groups_by_level_id(db, id)


@router.get("/groups/", response_model=Iterable[Group])
async def get_all_groups_view(db: AsyncSession = Depends(get_db)) -> Iterable[Group]:
